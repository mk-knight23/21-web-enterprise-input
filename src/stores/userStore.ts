  ...
  persist(
    set => ({
      ...
      updateProfile: updates =>
        set((state: any) => ({ profile: { ...state.profile, ...updates } })),
      updateSettings: updates =>
        set((state: any) => ({ settings: { ...state.settings, ...updates } })),
      incrementVisits: () =>
        set((state: any) => ({
          stats: { ...state.stats, totalVisits: state.stats.totalVisits + 1 },
        })),
      incrementProjects: () =>
        set((state: any) => ({
          stats: { ...state.stats, projectsCreated: state.stats.projectsCreated + 1 },
        })),
    }),
    { name: '${REPO_NAME}-user' }
  )
